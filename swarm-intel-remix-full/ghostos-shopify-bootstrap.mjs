#!/usr/bin/env node
/**
 * Full remix bootstrap: prefers GraphQL, falls back to REST, includes ngrok detection and CI-friendly behavior.
 */
import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';
import {
  listWebhooks, createWebhook, listPriceRules, createPriceRule, listDiscountCodes, createDiscountCode, listOrdersSince, ensureDiscountGraphQL
} from './lib/shopify.js';
const __dirname = path.dirname(fileURLToPath(import.meta.url));
function env(name, required = true) { const v = process.env[name]; if (required && !v) { console.error(`Missing env var ${name}. Create .env.example or export the variable.`); process.exit(1); } return v; }
...
async function ensureWebhooks(shop, token, runtimeUrl) {
  ...
  for (const r of required) {
    ...
    if (found) { console.log(`Webhook exists: ${r.topic} -> ${address} (id=${found.id})`); results.push({ topic: r.topic, address, created: false, id: found.id }); continue; }
...
async function ensureDiscount(shop, token, code, pct, expiresIso, limitToRepeatCustomers) {
  ...
  if (preferGraphql) {
    try {
      ...
      if (created) { console.log(`GraphQL discount created: id=${created.id} code=${created.code}`); return { graphql: true, discount: created }; }
    } catch (gErr) {
  ...
  if (!matched) {
    ...
    const payload = { title, target_type: 'line_item', target_selection: 'all', allocation_method: 'across', value_type: 'percentage', value: `-${pct.toString()}`, customer_selection: limitToRepeatCustomers ? 'prerequisite_customer_ids' : 'all', starts_at: new Date().toISOString(), ends_at: expiresIso, usage_limit: null, once_per_customer: true };
    matched = await createPriceRule(shop, token, payload);
    console.log(`Created price_rule id=${matched.id}`);
  } else { console.log(`Found existing price_rule id=${matched.id}`); }
  ...
  if (foundCode) { console.log(`Discount code already exists: ${foundCode.code} (id=${foundCode.id})`); return { graphql: false, price_rule_id: matched.id, discount_code: foundCode }; }
...
async function fetchRecentOrders(shop, token, days = 7) { const since = isoDaysAgo(days); const orders = await listOrdersSince(shop, token, since); return orders.map(o => ({ id: o.id, name: o.name, created_at: o.created_at, total_price: o.total_price, email: o.email, cart_token: o.cart_token || null, checkout_id: o.checkout_id || null })); }
...
async function main() {
  ...
  const SHOP = env('SHOP'); const ADMIN_TOKEN = env('ADMIN_TOKEN'); const RUNTIME_URL = env('RUNTIME_URL'); const DISCOUNT_CODE = env('DISCOUNT_CODE') || 'GHOST15-R3'; const DISCOUNT_PCT = parseFloat(process.env.DISCOUNT_PCT || '15'); const DISCOUNT_EXPIRES = env('DISCOUNT_EXPIRES'); const LIMIT_TO_REPEAT_CUSTOMERS = (process.env.LIMIT_TO_REPEAT_CUSTOMERS || 'false').toLowerCase() === 'true';
  console.log('Starting GhostOS remix-full bootstrap...');
  try {
    ...
    console.log('--- SUMMARY ---');
    for (const w of webhookResults) console.log(`  - ${w.topic} -> ${w.address} (id=${w.id})${w.created ? ' [created]' : ''}`);
    if (discountResult.graphql) console.log(`Discount (GraphQL): id=${discountResult.discount.id} code=${discountResult.discount.code}`); else console.log(`Discount: code=${discountResult.discount_code ? discountResult.discount_code.code : discountResult.discount?.code} price_rule_id=${discountResult.price_rule_id || 'n/a'}`);
    console.log(`Recent orders (last ${recentOrders.length}):`); console.log(JSON.stringify(recentOrders, null, 2));
    console.log('Bootstrap complete. Verify webhooks in Shopify Admin and test runtime endpoint with HMAC-signed payloads.');
  } catch (err) { console.error('Bootstrap failed:', err?.response || err?.message || err); process.exit(2); }
}
